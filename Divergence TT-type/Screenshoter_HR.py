...
def screenshoter_HR(timeinterval:str, symbol:str, atr1: float, atr2: float, atr3: float):
	...
	# --- PLOTS ---
	plt.suptitle(symbol + ' at ' + datetime.datetime.now().strftime('%H:%M:%S') + f" ({timeinterval})")
	...
	# SAVE AND SEND
	plt.savefig(f'HR{symbol}{timeinterval}.png', dpi=400, bbox_inches='tight', pad_inches=0.2)
	pic = open(f'HR{symbol}{timeinterval}.png', 'rb')
	bot2.send_photo(662482931, pic)

	# CLEANING
	pic.close()
	remove(f'HR{symbol}{timeinterval}.png')
	plt.cla()
	plt.clf()
